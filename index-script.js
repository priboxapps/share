// ==================== PRIBOX APP - WITH VAULT UNLOCK & FOOTER NOTIFICATIONS ====================
class PriBoxApp {
    constructor() {
        // Navbar properties
        ...
        this.secureModules = ['photos', 'notes', 'settings'];
    ...
    // ==================== INITIALIZATION ====================
    init() {
        console.log('Initializing PriBox App with vault unlock...');
        this.initToastBar();
        ...
        this.loadUserData().then((hasUser) => {
            if (hasUser) {
                ...
                console.log('PriBox App initialized with user data');
            }
                    ...
                    <div class="apk-footer-section">
                        <div class="apk-footer-content">
                            <div class="apk-info">
                                ...
                                <div class="apk-text">
                                    <h4>PriBox Mobile App</h4>
                                    <p>Secure sharing • End-to-end encryption • Self-destructing content</p>
                                </div>
                            </div>
                            <a href="https://priboxapps.github.io/share/PriBox.apk" id="homeApkDownloadLink" class="apk-download-btn" download>
                                <span class="material-icons">download</span> Download PriBox APK
                            </a>
                        </div>
                    </div>
                </div>
    ...
    attachHomeEventListeners() {
        ...
        if (apkDownloadBtn) {
            apkDownloadBtn.addEventListener('click', (e) => {
                console.log('APK download initiated from home page footer');
                this.showToastMessage('Downloading PriBox APK...', 'info');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded, initializing PriBox App');
    const checkAuthReady = setInterval(() => {
        if (window.authModule) {
            ...
            priBoxApp = new PriBoxApp();
            window.priBoxApp = priBoxApp;
        }
    }, 100);
});

window.addEventListener('popstate', function(event) {
    if (event.state && event.state.page && window.priBoxApp) {
        window.priBoxApp.showPage(`${event.state.page}-page`);
        const menuItem = document.querySelector(`.navbar-menu .menu-item[data-page="${event.state.page}"]`);
        if (menuItem) window.priBoxApp.setActiveMenuItem(menuItem);
    }
});
...
window.toastManager = {
    show: (message, type = 'info') => {
        if (window.priBoxApp && window.priBoxApp.showToastMessage) {
            window.priBoxApp.showToastMessage(message, type);
        } else {
            console.log(`[${type.toUpperCase()}] ${message}`);
        }
    },
    init: () => {
        if (window.priBoxApp && window.priBoxApp.initToastBar) {
            window.priBoxApp.initToastBar();
        }
    },
    getTitle: (type) => {
        if (window.priBoxApp && window.priBoxApp.getToastTitle) {
            return window.priBoxApp.getToastTitle(type);
        }
        return 'Notification';
    }
};
