...
class CredentialManager {
        ...
        try {
            ...
            if (firebaseData && Object.keys(firebaseData).length > 0) {
                ...
                for (const { data } of sortedRows) {
                    // CRITICAL FIX: Preserve pending status for each field
                    entries.push({ 
                        value: data.serviceTag || '', 
                        display: data.serviceTag || '', 
                        pending: data.serviceTagPending || data.pending || false,  // FIX: Check field-specific pending
                        pendingAt: data.serviceTagPendingAt || data.pendingAt || null, 
                    ...
                    entries.push({ 
                        ...
                        pending: data.usernamePending || data.pending || false,  // FIX: Check field-specific pending
                        pendingAt: data.usernamePendingAt || data.pendingAt || null, 
                    ...
                    entries.push({ 
                        ...
                        pending: data.passwordPending || data.pending || false,  // FIX: Check field-specific pending
                        pendingAt: data.passwordPendingAt || data.pendingAt || null, 
                    ...
                    entries.push({ 
                        value: data.note || '', 
                        display: data.note || '', 
                        pending: data.notePending || data.pending || false,  // FIX: Check field-specific pending
                        pendingAt: data.notePendingAt || data.pendingAt || null, 
                        isEmpty: !data.note, 
                        isWhitespaceOnly: false, 
                        originalIndex: entries.length, 
                        lineNumber: Math.floor(entries.length / 6) + 1 
                    ...
                    entries.push({ 
                        value: data.twofa || '', 
                        display: data.twofa || '', 
                        pending: data.twofaPending || data.pending || false,  // FIX: Check field-specific pending
                        pendingAt: data.twofaPendingAt || data.pendingAt || null, 
                        isEmpty: !data.twofa, 
                        isWhitespaceOnly: false, 
                        originalIndex: entries.length, 
                        lineNumber: Math.floor(entries.length / 6) + 1 
                    ...
                    entries.push({ 
                        ...
                        pending: data.customFieldPending || data.pending || false,  // FIX: Check field-specific pending
                        pendingAt: data.customFieldPendingAt || data.pendingAt || null, 
        ...
        if (data && typeof data === 'object') {
            ...
            if (rowIds.length > 0) {
                ...
                for (const { rowData } of sortedRows) {
                    entries.push({ value: rowData.serviceTag || '', display: rowData.serviceTag || '', pending: rowData.pending || false, pendingAt: rowData.pendingAt || null, isEmpty: !rowData.serviceTag, isWhitespaceOnly: false, originalIndex: entries.length, lineNumber: Math.floor(entries.length / 6) + 1 });
                    entries.push({ value: rowData.username || '', display: rowData.username || '', pending: rowData.pending || false, pendingAt: rowData.pendingAt || null, isEmpty: !rowData.username, isWhitespaceOnly: false, originalIndex: entries.length, lineNumber: Math.floor(entries.length / 6) + 1 });
                    entries.push({ value: rowData.password || '', display: rowData.password || '', pending: rowData.pending || false, pendingAt: rowData.pendingAt || null, isEmpty: !rowData.password, isWhitespaceOnly: false, originalIndex: entries.length, lineNumber: Math.floor(entries.length / 6) + 1 });
                    entries.push({ value: rowData.note || '', display: rowData.note || '', pending: rowData.pending || false, pendingAt: rowData.pendingAt || null, isEmpty: !rowData.note, isWhitespaceOnly: false, originalIndex: entries.length, lineNumber: Math.floor(entries.length / 6) + 1 });
                    entries.push({ value: rowData.twofa || '', display: rowData.twofa || '', pending: rowData.pending || false, pendingAt: rowData.pendingAt || null, isEmpty: !rowData.twofa, isWhitespaceOnly: false, originalIndex: entries.length, lineNumber: Math.floor(entries.length / 6) + 1 });
                    entries.push({ value: rowData.customField || '', display: rowData.customField || '', pending: rowData.pending || false, pendingAt: rowData.pendingAt || null, isEmpty: !rowData.customField, isWhitespaceOnly: false, originalIndex: entries.length, lineNumber: Math.floor(entries.length / 6) + 1 });
                }
    ...
    async saveAllRowsToFirebase() {
        ...
        try {
            ...
            for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
                ...
                // CRITICAL FIX: Save pending status for each individual field
                const rowData = {
                    ...
                    // Save individual pending status for each field
                    serviceTagPending: entries[startIndex]?.pending || false,
                    usernamePending: entries[startIndex + 1]?.pending || false,
                    passwordPending: entries[startIndex + 2]?.pending || false,
                    twofaPending: entries[startIndex + 3]?.pending || false,
                    customFieldPending: entries[startIndex + 4]?.pending || false,
                    notePending: entries[startIndex + 5]?.pending || false,
                    // Save individual pending timestamps
                    serviceTagPendingAt: entries[startIndex]?.pendingAt || null,
                    usernamePendingAt: entries[startIndex + 1]?.pendingAt || null,
                    passwordPendingAt: entries[startIndex + 2]?.pendingAt || null,
                    twofaPendingAt: entries[startIndex + 3]?.pendingAt || null,
                    customFieldPendingAt: entries[startIndex + 4]?.pendingAt || null,
                    notePendingAt: entries[startIndex + 5]?.pendingAt || null,
                    // Legacy pending field for backward compatibility
            ...
            console.log(`All ${rows} credential rows saved to Firebase with pending status preserved`);
            return true;
    ...
    // Sync to Firebase
// Fix syncRowToFirebase - ensure correct field order
async syncRowToFirebase(rowIndex) {
    ...
    const rowData = {
        ...
        username: entries[startIndex + 1]?.value || '',
        password: entries[startIndex + 2]?.value || '',
        twofa: entries[startIndex + 3]?.value || '',      // 2FA at index 3
        customField: entries[startIndex + 4]?.value || '', // Custom Field at index 4
        note: entries[startIndex + 5]?.value || '',       // Note at index 5
        // Save individual pending status
        serviceTagPending: entries[startIndex]?.pending || false,
        usernamePending: entries[startIndex + 1]?.pending || false,
        passwordPending: entries[startIndex + 2]?.pending || false,
        twofaPending: entries[startIndex + 3]?.pending || false,
    ...
    // Remove operation marker after a delay
    setTimeout(() => {
    ...
    // Fix syncRowToFirebase - Note should be at index 5, not 3
    async syncRowToFirebase(rowIndex) {
        if (!window.authModule || !window.authModule.isLoggedIn()) {
            return false;
        }
        ...
        const rowData = {
            ...
            twofa: entries[startIndex + 3]?.value || '',      // 2FA at index 3
            customField: entries[startIndex + 4]?.value || '', // Custom Field at index 4
            note: entries[startIndex + 5]?.value || '',       // Note at index 5 (FIXED)
            // Save individual pending status
            serviceTagPending: entries[startIndex]?.pending || false,
            usernamePending: entries[startIndex + 1]?.pending || false,
            passwordPending: entries[startIndex + 2]?.pending || false,
            twofaPending: entries[startIndex + 3]?.pending || false,
            customFieldPending: entries[startIndex + 4]?.pending || false,
            notePending: entries[startIndex + 5]?.pending || false,  // Note pending at index 5
            // Save individual pending timestamps
            serviceTagPendingAt: entries[startIndex]?.pendingAt || null,
            usernamePendingAt: entries[startIndex + 1]?.pendingAt || null,
            passwordPendingAt: entries[startIndex + 2]?.pendingAt || null,
...
createDeleteConfirmationBar(rowIndex, serviceName, rowNumber) {
    ...
    div.innerHTML = `
        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                <input type="text" class="delete-confirm-input form-input" placeholder='Type "DELETE" to confirm'
                    style="border: 1px solid rgba(239, 68, 68, 0.3); width: 140px;">
                ...
                <div class="form-help">Type "DELETE" to confirm*</div>
            </div>
        </div>
    ...
    const input = div.querySelector('.delete-confirm-input');
    ...
    confirmBtn.addEventListener('click', () => {
        ...
        if (confirmText.toUpperCase() === 'DELETE') {
            ...
            errorDiv.textContent = 'Please type "DELETE" exactly to confirm deletion';
...
addDeleteStyles() {
                ...
                <form class="settings-form" id="credentialForm">
                    ...
                    <div style="margin: 4px 0; padding-top: 8px; border-top: 1px solid var(--border);">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; user-select: none;">
                            <input type="checkbox" 
                                id="moreOptionsCheckbox" 
                                style="width: 18px; height: 18px; cursor: pointer; color: var(--primary);"
                                onchange="credentialManager.toggleMoreOptions()">
                            <span style="color: var(--text-secondary); font-size: 0.85rem; font-weight: 500;">
                                <span class="material-icons" style="font-size: 16px; vertical-align: middle;">expand_more</span>
                    ...
                    <div id="extraFieldsContainer" style="display: none;">
                        <div class="form-row">
                            ...
                            <!-- 6. CUSTOM FIELD WITH INLINE EDIT -->
                            <div class="form-group">
                                ...
                                <div class="password-input-group">
                                    <input type="password" 
                                        ...
                                        autocomplete="off">
                                    <button type="button" class="toggle-password-btn" data-target="formCustomField">
                                        <span class="material-icons">visibility</span>
                                    </button>
                                </div>
    ...
    toggleMoreOptions() {
        const checkbox = document.getElementById('moreOptionsCheckbox');
        const extraContainer = document.getElementById('extraFieldsContainer');
        
        if (checkbox && extraContainer) {
            const isChecked = checkbox.checked;
            extraContainer.style.display = isChecked ? 'block' : 'none';
            
            // Store preference in localStorage
            localStorage.setItem('showMoreOptions', isChecked ? 'true' : 'false');
            
            // Optional: animate smooth transition
            if (isChecked) {
    ...
    // Load saved More Options preference
    loadMoreOptionsPreference() {
        const saved = localStorage.getItem('showMoreOptions');
        const checkbox = document.getElementById('moreOptionsCheckbox');
        const extraContainer = document.getElementById('extraFieldsContainer');
        ...
        if (checkbox && extraContainer) {
            ...
            extraContainer.style.display = shouldShow ? 'block' : 'none';
        }
    }
...
credentialHTML() {
    ...
    return `
        <div class="table-wrapper" style="
            ...
            margin: 16px 0;
        ">
            <!-- Toggle Button for Credential Form -->
...
addCredentialForm(formData) {
    ...
    const newEntries = [
        ...
        { value: formData.customField, display: formData.customField, pending: false, pendingAt: null, isEmpty: !formData.customField, isWhitespaceOnly: false, originalIndex: startIndex + 4, lineNumber: newRowNumber },
...
attachFormEventListeners() {
    ...
    this.loadMoreOptionsPreference();
}
...
getPreviewSectionHTML() {
    ...
    getPreviewContentHTML() {
        ...
        for (const { rowIndex, rowEntries } of filteredRows) {
            ...
            // Get service color for non-pending rows
            //const serviceColor = (!isRowPending && serviceInfo) ? serviceInfo.color : null;
            
            // Field order: 0:Service, 1:Username, 2:Password, 3:2FA, 4:customField, 5:Note
            const orderedFields = [
                //{ index: 0, name: 'Service', isSecret: false, col: 0 },  // ADD THIS
                { index: 1, name: 'Username', isSecret: false, col: 1 },
                { index: 2, name: 'Password', isSecret: true, col: 2 },
                { index: 3, name: '2FA', isSecret: true, col: 3 },
                { index: 4, name: this.getCustomFieldLabel(), isSecret: true, col: 4 },
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px;">
                        ${orderedFields.map(field => {
                            ...
                            if (is2FA) {
                                ...
                                hasTimer = true;
                            } else if (isSecretField && !this.showPasswords && !isPending && entry.value && entry.value.trim()) {
                                displayValue = '••••••••••';
                                copyValue = entry.value;
                            } else if (isNote) {
                                // Note field: max 24 chars, no expansion
                            ...
                            return `
                                <div class="preview-item ${hasTimer ? 'totp-display' : ''}" 
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: ${hasTimer ? 'center' : 'flex-start'}; height: ${hasTimer ? '100%' : 'auto'};">
                                        ...
                                        ${!hasTimer && isSecretField && !isPending && entry.value && entry.value.trim() ? `
...
toggleRowStatus(rowIndex) {
    ...
    if (credentialIndex !== -1) {
        ...
        // Check if row has any data (non-empty values) - check all fields
        ...
        // Check current pending status of the row
        const hasPending = entries.slice(startIndex, startIndex + 6)
            .some(entry => entry && entry.pending);
        
        const newPendingStatus = !hasPending;
        const fieldNames = ['Service Tag', 'Username', 'Password', '2FA', 'Custom Field', 'Note'];
        
        console.log(`Toggling row ${rowIndex + 1} from ${hasPending ? 'pending' : 'active'} to ${newPendingStatus ? 'pending' : 'active'}`);
        
        // Toggle each entry's pending status - ALL 6 fields (indices 0-5)
        for (let i = 0; i < 6; i++) {
            ...
            if (entryIndex < entries.length && entries[entryIndex]) {
                ...
                console.log(`  Field ${i} (${fieldNames[i]}) pending: ${newPendingStatus}, value: "${(entries[entryIndex].value || '').substring(0, 20)}"`);
            }
        }
