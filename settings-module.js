...
class SettingsModule {
    ...
    ensureNavbarActiveState() {
        if (window.priBoxApp && window.priBoxApp.currentPage) {
            const activePage = window.priBoxApp.currentPage;
            const menuItem = document.querySelector(`.navbar-menu .menu-item[data-page="${activePage}"]`);
            if (menuItem && window.priBoxApp.setActiveMenuItem) {
                window.priBoxApp.setActiveMenuItem(menuItem);
            }
        }
    }
                        ...
                        <div class="settings-section" id="email-section">
                            ...
                            <div class="settings-card">
                                ...
                                <div class="verification-status">
                                    <div class="status-item ${userEmail ? 'verified' : 'unverified'}">
                                        <span class="material-icons">${userEmail ? 'check_circle' : 'cancel'}</span>
                                        <div>
                                            <h4>Current Email</h4>
                                            <p>${userEmail}</p>
                                            <small>${userEmail ? 'Verified and active' : 'Not verified'}</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    updateVerificationStatus() {
        ...
        const verificationContainer = document.querySelector('.verification-status');
        ...
        const statusHtml = `
            <div class="status-item verified">
                ...
                <div>
                    <h4>Current Email</h4>
                    <p>${userEmail}</p>
                    <small>Verified and active</small>
                </div>
            </div>
        `;
        
        verificationContainer.innerHTML = statusHtml;
    }
...
Generated by PriBox - ${new Date().toISOString()}
`;
